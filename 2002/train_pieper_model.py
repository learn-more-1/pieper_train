"""
训练基于隐式神经场的IK模型（2002优化版本）

相比2001的优化:
1. 增大隐藏层维度: 1000 -> 2000
2. 增加位置编码频率: 10 -> 15
3. 增加集成模型数量: 5 -> 8
4. 提高FK损失权重: 0.5 -> 1.0 (更关注末端位置精度)
5. 启用条件输入: use_condition=True (使用当前关节角帮助连续性)
"""
...
sys.path.insert(0, '/home/bonuli/Pieper/2002')
...
class Config:
    ...
    pose_dim = 7
    joint_dim = 7
    hidden_dim = 2000       # 1000 -> 2000 增加模型容量
    num_freqs = 15          # 10 -> 15 增加位置编码频率
    use_condition = True    # False -> True 启用条件输入（使用当前关节角）
    use_ensemble = True
    num_models = 8          # 5 -> 8 增加集成模型数量

    # 损失权重 - 更关注位置精度
    ik_weight = 1.0         # 关节角度损失
    fk_weight = 1.0         # 0.5 -> 1.0 提高FK权重，更关注末端位置精度
    consistency_weight = 0.1  # 一致性损失

    # 每个关节的损失权重（保持2001的设置）
    joint_loss_weights = torch.tensor([
        1.6,   # J0 (Shoulder Pitch): 16.30°
        2.0,   # J1 (Shoulder Roll):  20.32°
        1.6,   # J2 (Shoulder Yaw):   16.08°
        2.5,   # J3 (Elbow):          25.44°
        1.0,   # J4 (Forearm Roll):   10.53°
        0.7,   # J5 (Wrist Yaw):       7.43°
        0.6,   # J6 (Wrist Pitch):    6.12°
    ])
    # ================================================
...
def train():
    ...
    logging.info("=" * 70)
    logging.info("训练基于隐式神经场的IK模型（2002优化版本）")
    logging.info("=" * 70)
    logging.info("核心特性:")
    logging.info("  1. 隐式神经场：位姿 -> 关节角直接映射")
    logging.info("  2. 位置编码：多尺度正弦/余弦特征")
    logging.info("  3. 无需历史帧信息")
    logging.info(f"  4. 使用条件: {config.use_condition} (启用连续性)")
    logging.info(f"  5. 使用集成: {config.use_ensemble} ({config.num_models}个模型)")
    logging.info("  6. 带关节权重：针对误差大的关节增加损失权重")
    logging.info("\n2002优化配置:")
    logging.info(f"  - hidden_dim: {config.hidden_dim} (2001: 1000)")
    logging.info(f"  - num_freqs: {config.num_freqs} (2001: 10)")
    logging.info(f"  - num_models: {config.num_models} (2001: 5)")
    logging.info(f"  - fk_weight: {config.fk_weight} (2001: 0.5)")
    ...
    # 断点路径
    checkpoint_path = f"/home/bonuli/Pieper/2002/{model_name}_2002_with_con.pth"
