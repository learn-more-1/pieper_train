...
class Config:
    # 数据配置
    data_path = "/home/wsy/Desktop/casual/ACCAD_CMU_merged_training_data.npz"  # SFU窗口化数据集
...
def train():
    ...
    checkpoint_path = f"/home/wsy/Desktop/casual/{model_name}_{loss_type}_film_081.pth"
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for batch_X, batch_y, batch_last_angle in train_loader:
            ...
            # 前向传播（返回中间特征用于耦合损失）
            # 传入目标角度 batch_y 作为 FiLM 条件
            if config.use_film:
                joint_angles, nodes = model(batch_X_flat, batch_y, return_intermediates=True)
            else:
