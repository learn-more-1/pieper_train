...
class WindowedIKDataset(Dataset):
    ...
    def __init__(self, data_path, use_augmentation=False, augmentation_level='light'):
        ...
        self.X_full = data['X']  # (N, window_size, 14) - 前7维人位姿 + 后7维机器人角度
        self.y = data['y']  # (N, 14 or 7) - 可能是14维(人位姿+机器人角度) 或 7维(仅机器人角度)

        # 使用后7维（机器人历史角度）作为输入
        self.X = self.X_full[:, :, 7:]  # (N, window_size, 7) - 机器人历史角度

        self.window_size = self.X.shape[1]
        self.input_dim = self.X.shape[2]  # 7维（机器人角度）
        self.output_dim = self.y.shape[1]  # 7维或14维
        self.num_samples = self.X.shape[0]

        # 检查y的格式
        self.y_has_pose = (self.output_dim == 14)  # y是否包含位姿信息

        logging.info(f"  样本数: {self.num_samples}")
        logging.info(f"  窗口大小: {self.window_size}")
        logging.info(f"  输入维度（机器人角度）: {self.input_dim}")
        if self.y_has_pose:
            logging.info(f"  输出维度（人位姿+机器人角度）: {self.output_dim}")
        else:
            logging.info(f"  输出维度（仅机器人角度）: {self.output_dim}")
            logging.warning(f"  警告: y不包含位姿信息，需要使用FK计算")
    ...
    def __getitem__(self, idx):
        """
        返回:
            X: (window_size, 7) - 机器人历史关节角度
            y: (7,) or (14,) - 机器人角度，或(人位姿+机器人角度)
            last_angle: (7,) - 最后一帧的机器人关节角度
        """
        X = self.X[idx].astype(np.float32)  # (window_size, 7) - 机器人历史角度
        y = self.y[idx].astype(np.float32)  # (7,) or (14,)
        last_angle = self.X_full[idx, -1, 7:].astype(np.float32)  # (7,) - 最后一帧的机器人角度
