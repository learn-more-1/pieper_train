...
# 训练 FiLM 版本
python train_pieper_model.py film pieper_film

# 训练直接相乘版本
python train_pieper_model.py direct pieper_direct

# 使用默认名称（pieper_film 或 pieper_direct）
python train_pieper_model.py film
python train_pieper_model.py direct
...
import os
# 导入三种模型
from pieper_sesw import PieperCausalIK as PieperCausalIKFiLM  # FiLM版本（shoulder-es-wristyaw）
from pieper_direct import PieperCausalIKDirect  # 直接相乘版本
# sys.path.insert(0, '/home/wsy/Desktop/casual')
sys.path.insert(0, '/home/wsy/Desktop/casual/pieper1001')
...
class Config:
    ...
    hidden_dim = 512
    num_layers = 2
    model_type = "film"  # 模型类型: "film" 或 "direct"

    # 损失权重
    ik_weight = 0.0        # 关节角度损失（禁用！）
    fk_weight = 1.0        # 位置误差损失（主要）
    continuity_weight = 0.1  # 连续性损失
...
def train():
    ...
    # 解析命令行参数
    # 第一个参数: 模型类型 (film/direct)
    # 第二个参数: 模型名称（用于保存checkpoint）
    if len(sys.argv) > 1:
        config.model_type = sys.argv[1].lower()
    if len(sys.argv) > 2:
        model_name = sys.argv[2]
    else:
        model_name = f'pieper_{config.model_type}'

    # 验证模型类型
    if config.model_type not in ['film', 'direct']:
        logging.error(f"未知的模型类型: {config.model_type}")
        logging.error("请使用: python train_pieper_model.py [film|direct] [model_name]")
        return

    logging.info("=" * 70)
    logging.info(f"训练 Pieper 因果IK模型 - {config.model_type.upper()} 版本")
    logging.info("=" * 70)

    if config.model_type == 'film':
        logging.info("模型架构: FiLM 调制 (gamma * feat + beta)")
    else:
        logging.info("模型架构: 直接相乘交互 (feat * pose)")

    logging.info("核心特性:")
    logging.info("  1. Attention学习历史帧中的关键信息")
    logging.info("  2. Shoulder-ES-WristYaw 分组处理")
    if config.model_type == 'film':
        logging.info("  3. FiLM: 目标位姿 → gamma/beta → 调制历史特征")
    else:
        logging.info("  3. 直接相乘: 历史特征 * 目标位姿 → MLP")
    ...
    # 创建模型
    logging.info(f"\n创建模型: {model_name} ({config.model_type})")

    if config.model_type == 'film':
        model = PieperCausalIKFiLM(
    ...
    path = f"/home/wsy/Desktop/casual/pieper1003/{model_name}_v2.pth"
    logging.info(f"模型保存路径: {path}")
    # 断点路径
