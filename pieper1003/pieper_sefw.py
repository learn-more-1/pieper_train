...
class JointwiseAttentionEncoder(nn.Module):
    ...
    def __init__(self, num_frames=10, hidden_dim=256, num_heads=4):
        ...
        self.attention_encoders = nn.ModuleDict({
            ...
            'elbow': PieperAttention(input_dim=1, hidden_dim=hidden_dim, num_heads=num_heads, num_frames=num_frames),
            'forearm': PieperAttention(input_dim=1, hidden_dim=hidden_dim, num_heads=num_heads, num_frames=num_frames),
            'wrist': PieperAttention(input_dim=2, hidden_dim=hidden_dim, num_heads=num_heads, num_frames=num_frames)
        })
    ...
    def forward(self, history_frames):
        ...
        history_elbow = history_frames[:, :, 3:4]     # [batch, num_frames, 1]
        history_forearm = history_frames[:, :, 4:5]   # [batch, num_frames, 1]
        history_wrist = history_frames[:, :, 5:7]     # [batch, num_frames, 2]

        # 分别用注意力提取特征
        features = {
            'shoulder': self.attention_encoders['shoulder'](history_shoulder),
            'elbow': self.attention_encoders['elbow'](history_elbow),
            'forearm': self.attention_encoders['forearm'](history_forearm),
            'wrist': self.attention_encoders['wrist'](history_wrist)
        }
...
class PieperCausalIK(nn.Module):
    ...
    def forward(self, history_frames, end_position, end_orientation):
        ...
        pred_elbow = self.output_heads['elbow'](modulated_features['elbow'])
        pred_forearm = self.output_heads['forearm'](modulated_features['forearm'])
        pred_wrist = self.output_heads['wrist'](modulated_features['wrist'])
...
if __name__ == '__main__':
    ...
    print("    - elbow: [batch, 10, 1] → MultiHeadAttn → [batch, 256]")
    print("    - forearm: [batch, 10, 1] → MultiHeadAttn → [batch, 256]")
    print("    - wrist: [batch, 10, 2] → MultiHeadAttn → [batch, 256]")
    print("  【第二阶段】FiLM调制:")
