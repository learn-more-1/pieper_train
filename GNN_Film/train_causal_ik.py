...
sys.path.insert(0, '/home/wsy/Desktop/casual/GNN_Film')

from dataset_generalized import create_windowed_dataloaders
from causal_ik_model_film import PhysicsAwareCausalIK, PhysicsAwareLoss, PositionBasedLoss, PhysicsAwareCausalIKWithHistory
import pinocchio as pin

logging.basicConfig(
    level=logging.INFO,
    format="%(message)s",
    handlers=[logging.FileHandler("causal_train.log"), logging.StreamHandler()]
)


class Config:
    # 数据配置
    data_path = "/home/wsy/Desktop/casual/merged_training_data.npz"  # 合并后的数据集
    input_dim = 7    # 7维位姿（位置+姿态）
    output_dim = 7   # 7维关节角度（或14维：7位姿+7角度）
...
def forward_kinematics_with_pose(pin_model, pin_data, joint_ids, wrist_frame_id, joint_angles):
    """
    批量计算正向运动学（包括位置和姿态）
    ...
    Returns:
        ...
        orientations: [batch, 4] 手腕姿态（四元数 [x, y, z, w]）
    """
    batch_size = joint_angles.shape[0]
    device = joint_angles.device

    # 转换为numpy（Pinocchio需要）
    joint_angles_np = joint_angles.detach().cpu().numpy()
    ...
    for i in range(batch_size):
        ...
        # 获取姿态（旋转矩阵 -> 四元数）
        wrist_rot = pin_data.oMf[wrist_frame_id].rotation.copy()
        # 使用 scipy 将旋转矩阵转换为四元数
        from scipy.spatial.transform import Rotation as R
        rotation = R.from_matrix(wrist_rot)
    ...
    # 转回tensor
    return (
        torch.from_numpy(positions).to(device).float(),
        torch.from_numpy(orientations).to(device).float()
    )
...
def train():
    ...
    checkpoint_path = f"/home/wsy/Desktop/casual/{model_name}_{loss_type}_film.pth"
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for batch_X, batch_y, batch_last_angle in train_loader:
            ...
            # 前向传播：使用历史帧和目标位姿
            # 注意：PhysicsAwareCausalIKWithHistory 期望 [batch, 70] 格式
            batch_X_flat = history_frames.view(batch_size, -1)  # [batch, 70]

            # 传入目标角度作为 FiLM 条件
            if config.use_film:
                joint_angles, nodes = model(batch_X_flat, target_angles, return_intermediates=True)
            else:
                joint_angles, nodes = model(batch_X_flat, None, return_intermediates=True)
            ...
            if config.use_position_loss and pin_model is not None:
                ...
                true_wrist_pos_fk = forward_kinematics_batch(
                    pin_model, pin_data, joint_ids, wrist_frame_id, target_angles
                )

                # 3. 计算位置误差为主损失
                total_loss, loss_dict = loss_fn(
                    pred_wrist_pos, true_wrist_pos_fk,  # 都用 Pinocchio 计算
                    joint_angles, target_angles, nodes
                )
        ...
        with torch.no_grad():
            ...
            for batch_X, batch_y, batch_last_angle in val_loader:
                ...
                batch_X_flat = history_frames.view(batch_size, -1)  # [batch, 10*7]

                # 前向传播（传入目标角度作为 FiLM 条件）
                if config.use_film:
                    joint_angles, nodes = model(batch_X_flat, target_angles, return_intermediates=True)
                else:
                    joint_angles, nodes = model(batch_X_flat, None, return_intermediates=True)
