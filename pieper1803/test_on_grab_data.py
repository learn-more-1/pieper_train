...
from train_distillation_vae_ik import VAEIKModel
...
def test_on_grab_data():
    ...
    checkpoint = torch.load("/home/bonuli/Pieper/pieper1803/distillation_vae_ik_1803.pth")
    ...
    with torch.no_grad():
        for batch_idx, (batch_X, batch_y, batch_last_angle, batch_human_pose) in enumerate(loader):
            ...
            # 预测
            pred_angles, _, _ = model(
                batch_human_pose,  # 完整的人臂位姿 (位置3 + 四元数4)
                batch_last_angle    # 历史角度
            )
